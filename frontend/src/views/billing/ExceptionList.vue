    ...
    <Card title="异常数据处理">
      <Table :columns="columns" :data="data" :loading="loading" :pagination="pagination">
        ...
        <template #action="{ record }">
          <Space>
            <Button type="text" size="small" @click="handleFix(record)">修复</Button>
            <Button type="text" size="small" @click="handleIgnore(record)">忽略</Button>
          </Space>
        </template>
      </Table>
    </Card>
...
const columns = [
  ...
  { title: '类型', dataIndex: 'exception_type', slotName: 'exception_type', width: 150 },
...
const handleIgnore = (record: any) => Message.info(`忽略：${record.description}`)
